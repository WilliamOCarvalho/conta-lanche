...
export default function SellersScreen() {
  ...
  const [name, setName] = useState(''), [pixKey, setPixKey] = useState(''), [contact, setContact] = useState(''), [observation, setObservation] = useState(''), [saving, setSaving] = useState(false);
  const load = useCallback(() => { void listVendors().then(setVendors).catch(() => Alert.alert('Erro', 'Não foi possível carregar os vendedores.')); }, []);
  useFocusEffect(load);
  const startNew = () => { setEditing(null); setName(''); setPixKey(''); setContact(''); setObservation(''); setModal(true); };
  const startEdit = (vendor: Vendor) => { setEditing(vendor); setName(vendor.name); setPixKey(vendor.pixKey ?? ''); setContact(vendor.contact ?? ''); setObservation(vendor.observation ?? ''); setModal(true); };
  const save = async () => {
    if (!name.trim()) { Alert.alert('Informe o nome', 'O nome do vendedor é obrigatório.'); return; }
    setSaving(true);
    try { await saveVendor({ id: editing?.id, name, pixKey, contact, observation, active: editing?.active ?? true }); setModal(false); load(); }
