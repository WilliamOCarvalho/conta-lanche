...
export default function PurchaseDetailScreen() {
  ...
  const remove = () => Alert.alert('Excluir compra?', 'A foto também será removida do aparelho.', [{ text: 'Cancelar', style: 'cancel' }, { text: 'Excluir compra', style: 'destructive', onPress: () => { void (async () => { try { const path = await deletePurchase(id); await removePhoto(path); router.back(); } catch { Alert.alert('Erro', 'Não foi possível excluir a compra.'); } })(); } }]);
